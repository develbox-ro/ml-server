...
def eval(model, test_generator, test_steps, history,start_datetime, finish_datetime, current_model_folder_name):
    ...
    output_file = os.path.join(os.getenv("MODELS_PATH"), current_model_folder_name, 'evaluation_results.txt')
    ...
    os.makedirs(os.path.join(os.getenv("MODELS_PATH"), current_model_folder_name, 'metrics'), exist_ok=True)
