...
current_model_folder_name = f'{start_datetime.strftime("%Y-%m-%d_%H-%M-%S")}_{config.MODEL}'
# Create a new directory for models
os.makedirs(f"{os.getenv('MODELS_PATH')}/{current_model_folder_name}", exist_ok=True)
# Define a file path where you want to save the test_paths and test_labels
output_file = f"{os.getenv('MODELS_PATH')}/{current_model_folder_name}/test_data.pkl"
...
with open(f"{os.getenv('MODELS_PATH')}/{current_model_folder_name}/config.json", 'w') as f:
...
# Save to csv
hist_csv_file = f"{os.getenv('MODELS_PATH')}/{current_model_folder_name}/history.csv"
